// CPNR/hardware/NoticeNKFADC500/NoticeNKFADC500.c (1/2)
...
// CPNR/hardware/NoticeNKFADC500/NoticeNKFADC500.c (2/2)
...
{
  return (int)USB3ReadReg(NKFADC500_VENDOR_ID, NKFADC500_PRODUCT_ID, sid, 0x20000010);
}
...
int NKFADC500read_DATA(int sid, int bcount, unsigned char * data, unsigned int timeout)
{
  ...
  if (timeout == 0)
    return USB3Read(NKFADC500_VENDOR_ID, NKFADC500_PRODUCT_ID, sid, count, 0x40000000, data);  
  else
    return USB3ReadTimeout(NKFADC500_VENDOR_ID, NKFADC500_PRODUCT_ID, sid, count, 0x40000000, timeout, data);
}
...
void NKFADC500flush_DATA(int sid)
{
  ...
  char data[10485760];
  int bcount; // (unsigned long -> int)
  int chunk;  // (unsigned long -> int)
  int slice;  // (unsigned long -> int)
  int i;
  ...
  for (i = 0; i < chunk; i++)
    NKFADC500read_DATA(sid, 10240, (unsigned char*)data, 1000); // (timeout 추가)

  if (slice)
    NKFADC500read_DATA(sid, slice, (unsigned char*)data, 1000); // (timeout 추가)
}
