...
///////////////////////////////////////////////////////////////////////////////
static void USB3Reset(uint16_t vendor_id, uint16_t product_id, int sid)
{
  unsigned char data;
...
int USB3Read(uint16_t vendor_id, uint16_t product_id, int sid, uint32_t count, uint32_t addr, unsigned char *data)
{
  ...
  // (CPNR) We use a fix-sized buffer on stack instead of malloc
  unsigned char local_buffer[16384]; // 16kB
  buffer = local_buffer;
  
  // (CPNR) Original code had a potential memory leak if malloc fails
  ...
  if (!devh) {
    ...
    // (CPNR) Bugfix: free(buffer) was missing in original code
    // free(buffer); 
    return -1;
  }
  ...
  if ((stat = libusb_bulk_transfer(devh, USB3_SF_WRITE, buffer, length, &transferred, timeout)) < 0) {
    ...
    USB3Reset(vendor_id, product_id, sid);
    // free(buffer); // (CPNR) No longer needed
    return stat;
  }

  for (loop = 0; loop < nbulk; loop++) {
    // (CPNR) Read directly into data buffer if possible, otherwise use intermediate
    ...
    // memcpy(data + loop * size, buffer, size); // (CPNR) No longer needed if reading directly
  }

  if (remains) {
    // (CPNR) Read remaining data into user buffer directly
    ...
    // memcpy(data + nbulk * size, buffer, remains * 4); // (CPNR) No longer needed
  }
...
// (CPNR) Added Timeout version from CUPDAQ/Notice/src/usb3com.cc
    ...
    uint32_t count, uint32_t addr, unsigned int timeout, unsigned char* data)
{
  ...
  libusb_device_handle* devh = nkusb_get_device_handle(vendor_id, product_id, sid);
  if (!devh) {
    fprintf(stderr, "USB3ReadTimeout: Could not get device handle for the device.\n");
    return -1;
  }
  ...
  if ((stat = libusb_bulk_transfer(devh,
           ...
           timeout))
      < 0) {
    fprintf(stderr, "USB3ReadTimeout: write error:%d [sid=%d]\n", stat, sid);
    return stat;
  }
  ...
  for (int loop = 0; loop < nbulk; loop++) {
    if ((stat = libusb_bulk_transfer(devh,
             ...
             timeout))
        < 0) {
      fprintf(stderr, "USB3ReadTimeout: read error:%d [sid=%d]\n", stat, sid);
      return stat;
    }
  }
...
unsigned int USB3ReadReg(uint16_t vendor_id, uint16_t product_id, int sid, uint32_t addr)
{
  ...
  if (status < 0) {
      fprintf(stderr, "USB3ReadReg: read error = %d\n", status);
      return status;
  }
...
// (CPNR) Added from CUPDAQ/Notice/src/usb3com.cc
...
unsigned long USB3ReadRegL(uint16_t vendor_id, uint16_t product_id, int sid, uint32_t addr)
{
  ...
  int status = USB3Read(vendor_id, product_id, sid, 2, addr, data);
    if (status < 0) {
      fprintf(stderr, "USB3ReadRegL: read error = %d\n", status);
      return status;
  }
